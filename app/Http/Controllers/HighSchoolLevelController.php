...
class HighSchoolLevelController extends Controller {
	...
	public function index()
	{
		try {
			...
			$previous_year = DB::select(DB::raw("SELECT SUM(a.total_boy + a.total_girl) AS previous_total_std, t.township_name, s.location, t.id FROM student_intake AS a INNER JOIN v_school AS s ON s.school_id = a.school_id INNER JOIN township AS t ON t.id = s.township_id AND a.school_year = '".Request::input('previous_year')."' and a.grade='10' AND s.state_divsion_id = ".Request::input('state_id')." AND (s.township_id = '".Request::input('township_id')."' OR '' = '".Request::input('township_id')."') GROUP BY s.township_id"));
			...
			return view('student_flow_rate.high_school_level', compact('record','region'));
