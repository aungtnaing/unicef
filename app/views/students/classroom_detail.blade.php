...
<div class="box inner-content">
	...
	<div class="row" style='margin:15px auto;'>
		<form action="{{ URL::route('calssroom_detail_list') }}" method="post" style="display:inline;" class="form-horizontal">
	...
	<table class="table table-bordered">
		...
		<?php if($c->location == "Rural" && $c->school_level == $rural_levels[$k]) { ?>
			<tr>
				...
				@foreach($walls as $w)
					@if($w->school_id == $c->school_id)
						<td>{{ $class = $w->permanent_wall + $w->temporary_wall }}</td>

						<td>
						@if($class)	
							{{ floor($totalstd/$class) }}
						@endif	
						</td>
					@endif
				@endforeach
